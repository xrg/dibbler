...
void add_radvd_conf(const char* ifname, const char* prefixPlain, int prefixLength,
    ...
    tmpf = NULL;
    if (f) {
        /* scan file, see if it's generated by dibbler, and locate first line to replace */
        fseek(f, 0, SEEK_SET);
        while (fgets(line, sizeof(line)-1, f)) {
                if (f_state == 0) {
                        /* looking for "generated by" line */
                        if (strstr(line, "generated by Dibbler")){
                                /* skip another 2 lines */
                                fgets(line, sizeof(line)-1, f);
                                fgets(line, sizeof(line)-1, f);
                                f_state = 1;
                                snprintf(needle, 63, "### %s start ###\n", ifname);
                                break;
                        }
                        start_pos = ftell(f);
                }
                else if (f_state == 1) {
                        /* looking for interface start line*/
                        if (!strcmp(line, needle)){
                                snprintf(needle, 63, "### %s end ###\n", ifname);
                                f_state = 2;
                        }
                        else
                            start_pos = ftell(f);
        ...
        if (!feof(f)) {
                ...
                if (tmpf){
                    ...
                    sprintf(errorMsg, "Cannot create tmp file to buffer radvd.conf");
                    return;
                }
        }
    } else {
    ...
    if (tmpf) {
            /* copy back remainder into 'radvd.conf' */
            fseek(tmpf, 0, SEEK_SET);
            while (fgets(line, sizeof(line)-1, tmpf))
                fputs(line, f);
            fclose(tmpf);
    }
